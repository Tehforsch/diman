use crate::example_system::dvec3::Velocity as Vec3Velocity;
use crate::example_system::f64::{Length, Time};
use glam::DVec2;
use glam::DVec3;
...
fn debug_vector_2() {
    assert_eq!(
        format!(
            "{:?}",
            crate::example_system::dvec2::Length::meters(1.0, 5.0)
        ),
        "[1, 5] m"
    );
    assert_eq!(
        format!(
            "{:?}",
            crate::example_system::dvec2::Length::kilometers(1.0, 5.0)
        ),
        "[1, 5] km"
    );
}
...
fn debug_vector_3() {
    assert_eq!(
        format!(
            "{:?}",
            crate::example_system::dvec3::Length::meters(1.0, 5.0, 6.0)
        ),
        "[1, 5, 6] m"
    );
}
